...
# For local/demo only; override via SEED_ADMIN_PASSWORD / SEED_DEMO_PASSWORD in production
...
class Command(BaseCommand):
    ...
    def handle(self, *args: Any, **kwargs: Any) -> None:
        ...
        if not User.objects.filter(email='demo@example.com').exists():
            User.objects.create_user(username='demo', email='demo@example.com', password=DEMO_PASSWORD)
